...
export default function Onboarding() {
  ...
  const createSchool = async (input: {
    ...
    if (input.contact) {
      const { error: contactError } = await supabase.rpc("save_school_contact_details", {
        p_school_id: (data as { id: string }).id,
        p_address: input.contact.address,
        p_contact_phone: input.contact.phone,
        p_contact_email: input.contact.email,
      });
      if (contactError) console.warn("Coordonnées professionnelles à enregistrer après application de la migration Schooly", contactError.message);
    }
  ...
  const finishAutomaticSignup = async (draft: SchoolySignupDraft, userId: string) => {
    ...
    try {
      await createSchool({
        ...
        contact: { address: draft.school.address, phone: draft.school.phone, email: draft.school.email },
      });
  ...
  if (automaticDraft) return <div className="onboarding-loading"><LoaderCircle size={25} /><strong>Bienvenue, {userName || automaticDraft.director.firstName}.</strong><span>{status === "saving" ? `Schooly crée ${automaticDraft.school.name}, ses accès et ses ${automaticDraft.classes.length} classes.` : message}</span>{status === "error" && <button className="auth-submit" onClick={() => void finishAutomaticSignup(automaticDraft, "retry")}>Réessayer</button>}</div>;
