...
export default function Home() {
  ...
  useEffect(() => { void (async () => { const { data: { user } } = await supabase.auth.getUser(); if (!user) { setLocation("/connexion"); return; } setUserName(String(user.user_metadata.full_name ?? user.email?.split("@")[0] ?? "Mon compte")); const { data: admin } = await supabase.from("platform_admins").select("user_id").eq("user_id", user.id).eq("status", "active").maybeSingle(); setIsPlatformAdmin(Boolean(admin)); const { data: schools } = await supabase.from("schools").select("id, name, slug, city, country, approval_status, rejection_reason").limit(1); if (!schools?.length) { setLoadingAccess(false); return; } setSchoolName(schools[0].name); setSchoolId(schools[0].id); setSchoolSlug(schools[0].slug); setApprovalStatus(schools[0].approval_status as "pending" | "approved" | "rejected" | "suspended"); setRejectionReason(schools[0].rejection_reason); setLoadingAccess(false); })(); }, [setLocation]);
