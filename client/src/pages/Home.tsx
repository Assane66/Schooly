...
function StudentsPage({ schoolId, onAction }: { schoolId: string; onAction: (name: string) => void }) {
  ...
  const [portalRelationship, setPortalRelationship] = useState<"student" | "guardian">("guardian");
  ...
  const exportStudents = () => { const escape = (value: string) => `"${value.replace(/"/g, '""')}"`; const rows = [["Matricule", "Prénom", "Nom", "Statut"], ...students.map((student) => [student.student_number, student.first_name, student.last_name, student.status])]; const csv = `\uFEFF${rows.map((row) => row.map(escape).join(",")).join("\n")}`; const link = document.createElement("a"); link.href = URL.createObjectURL(new Blob([csv], { type: "text/csv;charset=utf-8" })); link.download = `eleves-schooly-${new Date().toISOString().slice(0, 10)}.csv`; link.click(); URL.revokeObjectURL(link.href); toast.success("Export CSV téléchargé", { description: `${students.length} élève(s) exporté(s).` }); };
  const importStudents = async () => { if (!importFile) return; setImporting(true); const text = await importFile.text(); const lines = text.replace(/^\uFEFF/, "").split(/\r?\n/).filter(Boolean); if (lines.length < 2) { setImporting(false); toast.error("Le fichier ne contient aucun élève."); return; } const delimiter = lines[0].includes(";") ? ";" : ","; const headers = lines[0].split(delimiter).map((item) => item.trim().toLocaleLowerCase()); const firstNameIndex = headers.findIndex((header) => ["prénom", "prenom", "first_name"].includes(header)); const lastNameIndex = headers.findIndex((header) => ["nom", "last_name"].includes(header)); const numberIndex = headers.findIndex((header) => ["matricule", "student_number", "numéro", "numero"].includes(header)); if (firstNameIndex < 0 || lastNameIndex < 0) { setImporting(false); toast.error("Colonnes manquantes", { description: "Le fichier doit contenir au minimum les colonnes Prénom et Nom." }); return; } const records = lines.slice(1).flatMap((line, index) => { const values = line.split(delimiter).map((item) => item.trim().replace(/^"|"$/g, "")); const firstName = values[firstNameIndex]; const lastName = values[lastNameIndex]; return firstName && lastName ? [{ school_id: schoolId, first_name: firstName, last_name: lastName, student_number: numberIndex >= 0 && values[numberIndex] ? values[numberIndex] : `ELV-${new Date().getFullYear()}-${String(index + 1).padStart(4, "0")}-${crypto.randomUUID().slice(0, 4).toUpperCase()}`, status: "active" }] : []; }); if (!records.length) { setImporting(false); toast.error("Aucune ligne valide", { description: "Vérifiez les colonnes Prénom et Nom." }); return; } const { error } = await supabase.from("students").insert(records); setImporting(false); if (error) { toast.error("L’import n’a pas abouti", { description: error.message }); return; } setImportFile(null); setShowImport(false); await loadStudents(); toast.success("Import terminé", { description: `${records.length} élève(s) ajouté(s).` }); };
  ...
  const inviteToPortal = async (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!portalStudentId) return; setInvitingPortal(true); const { data: { user } } = await supabase.auth.getUser(); const { error } = await supabase.from("student_portal_invites").insert({ school_id: schoolId, student_id: portalStudentId, email: portalEmail.trim().toLowerCase(), relationship: portalRelationship, invited_by: user?.id ?? null }); setInvitingPortal(false); if (error) { toast.error("L’invitation n’a pas été créée", { description: error.message }); return; } const student = students.find((item) => item.id === portalStudentId); const studentName = student ? `${student.first_name} ${student.last_name}` : "Le dossier"; setPortalEmail(""); setPortalStudentId(""); setShowPortalInvite(false); toast.success("Invitation préparée", { description: `${studentName} sera associé à ce compte lors de sa création Schooly.` }); };
  return (
    <>
      ...
      <section className="module-summary-strip">
        ...
        <div className="module-summary-actions"><button className="soft-button" onClick={() => setShowPortalInvite(true)}>Inviter élève / famille <ArrowRight size={16} /></button><button className="soft-button" onClick={() => setShowImport(true)}>Importer CSV <ArrowUpRight size={16} /></button><button className="soft-button" onClick={exportStudents}><Download size={16} /> Exporter CSV</button></div>
      </section>
      {showImport && <section className="student-import-panel"><div><p className="eyebrow">IMPORT CSV</p><h2>Ajouter plusieurs élèves</h2><p>Colonnes requises : <strong>Prénom</strong> et <strong>Nom</strong>. La colonne <strong>Matricule</strong> est facultative.</p></div><div className="student-import-actions"><input type="file" accept=".csv,text/csv" onChange={(event) => setImportFile(event.target.files?.[0] ?? null)} /><small>{importFile ? importFile.name : "Sélectionnez un fichier CSV."}</small><div><button className="outline-button" onClick={() => { setShowImport(false); setImportFile(null); }}>Annuler</button><button className="primary-button" disabled={!importFile || importing} onClick={() => void importStudents()}>{importing ? "Import…" : "Importer les élèves"} <ArrowRight size={16} /></button></div></div></section>}
      {showPortalInvite && <section className="student-create-panel portal-invite-panel"><div><p className="eyebrow">ACCÈS INDIVIDUEL</p><h2>Inviter au portail élève & famille</h2><p>Le compte invité reste personnel. À sa création, il sera associé uniquement au dossier sélectionné.</p></div><form onSubmit={inviteToPortal}><label>Élève<select value={portalStudentId} onChange={(event) => setPortalStudentId(event.target.value)} required><option value="">Choisir un élève</option>{students.map((student) => <option value={student.id} key={student.id}>{student.first_name} {student.last_name} · {student.student_number}</option>)}</select></label><label>Type d’accès<select value={portalRelationship} onChange={(event) => setPortalRelationship(event.target.value as "student" | "guardian")}><option value="guardian">Responsable / famille</option><option value="student">Élève</option></select></label><label>Adresse e-mail<input type="email" value={portalEmail} onChange={(event) => setPortalEmail(event.target.value)} placeholder="famille@exemple.com" required /></label><div className="student-form-actions"><button type="button" className="outline-button" onClick={() => setShowPortalInvite(false)}>Annuler</button><button className="primary-button" disabled={invitingPortal || !students.length}>{invitingPortal ? "Préparation…" : "Préparer l’invitation"} <ArrowRight size={16} /></button></div></form></section>}
...
function ClassesPage({ schoolId, onAction }: { schoolId: string; onAction: (name: string) => void }) {
  const v1LevelHints = ["CI", "CP", "CE1", "CE2", "CM1", "CM2", "6e", "5e", "4e", "3e", "Seconde", "Première", "Terminale"];
  ...
  const createClass = async (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!activeYear) return; const unsupported = /(universit|facult|d[ée]partement|semestre|licence|master|doctorat|bachelor|cr[eé]dit)/i; if (unsupported.test(`${levelName} ${className}`)) { toast.error("Niveau non disponible en version initiale", { description: "Schooly v1 prend en charge le primaire, le collège et le lycée. Les structures universitaires seront proposées dans un parcours distinct." }); return; } setSaving(true); const { data, error } = await supabase.from("classes").insert({ school_id: schoolId, academic_year_id: activeYear.id, level_name: levelName.trim(), name: className.trim(), capacity: Number(capacity) || null }).select("id, academic_year_id, level_name, name, capacity").single(); setSaving(false); if (error || !data) { toast.error("La classe n’a pas été créée", { description: error?.message }); return; } setSchoolClasses((current) => [...current, data as SchoolyClass]); setLevelName(""); setClassName(""); setCapacity("30"); setShowClassForm(false); toast.success("Classe créée", { description: `${data.name} est disponible pour les inscriptions.` }); };
...
export default function Home() {
  ...
  useEffect(() => { void (async () => { const { data: { user } } = await supabase.auth.getUser(); if (!user) { setLocation("/connexion"); return; } setUserName(String(user.user_metadata.full_name ?? user.email?.split("@")[0] ?? "Mon compte")); setIsPlatformAdmin(await hasPlatformAdminRole()); const { data: schools } = await supabase.from("schools").select("id, name, slug, city, country, approval_status, rejection_reason").limit(1); if (!schools?.length) { setLoadingAccess(false); return; } const school = schools[0]; const { data: membership } = await supabase.from("school_memberships").select("role").eq("school_id", school.id).eq("user_id", user.id).maybeSingle(); const nextRole = membership?.role ?? "member"; setMemberRole(nextRole); const { data: assignments } = await supabase.from("school_member_custom_roles").select("role_id").eq("school_id", school.id).eq("user_id", user.id); const roleIds = (assignments ?? []).map((assignment) => assignment.role_id); const { data: permissions } = roleIds.length ? await supabase.from("school_role_permissions").select("permission").in("role_id", roleIds) : { data: [] as Array<{ permission: string }> }; setMemberPermissions((permissions ?? []).map((permission) => permission.permission)); setSchoolName(school.name); setSchoolId(school.id); setSchoolSlug(school.slug); setApprovalStatus(school.approval_status as "pending" | "approved" | "rejected" | "suspended"); setRejectionReason(school.rejection_reason); setLoadingAccess(false); })(); }, [setLocation]);
  ...
  const leaders = ["owner", "director", "administrator"];
  const nativePermissions: Record<string, string[]> = { accountant: ["payments.view", "payments.manage"], secretary: ["students.view", "students.manage", "registrations.manage", "classes.manage"], teacher: ["students.view", "attendance.manage", "grades.manage"], supervisor: ["attendance.manage"] };
  const requiredPermission: Record<string, string | undefined> = { "Élèves": "students.view", Inscriptions: "registrations.manage", Classes: "classes.manage", Pédagogie: "grades.manage", "Vie scolaire": "attendance.manage", Finances: "payments.view", Documents: "documents.manage", Communication: "roles.manage", Calendrier: "calendar.manage", "Équipe & rôles": "roles.manage" };
  const activePermissions = Array.from(new Set([...(nativePermissions[memberRole] ?? []), ...memberPermissions]));
  const availableNavGroups = navGroups.map((group) => ({ ...group, items: group.items.filter((item) => !requiredPermission[item.label] || leaders.includes(memberRole) || activePermissions.includes(requiredPermission[item.label]!)) })).filter((group) => group.items.length > 0);
  const memberRoleLabel = ({ owner: "Propriétaire", director: "Directeur", administrator: "Administrateur", accountant: "Comptable", secretary: "Secrétaire", teacher: "Professeur", supervisor: "Surveillant" } as Record<string, string>)[memberRole] ?? "Membre de l’équipe";
  const moduleMap: Record<string, { eyebrow: string; icon: typeof CalendarDays }> = { Pédagogie: { eyebrow: "GESTION SCOLAIRE", icon: BookOpenCheck }, "Vie scolaire": { eyebrow: "GESTION SCOLAIRE", icon: UserCheck }, Finances: { eyebrow: "ADMINISTRATION", icon: WalletCards }, Documents: { eyebrow: "ADMINISTRATION", icon: FileText }, Communication: { eyebrow: "ADMINISTRATION", icon: Bell }, Calendrier: { eyebrow: "ADMINISTRATION", icon: CalendarDays } };
  const renderContent = () => { if (activeNav === "Vue d’ensemble") return <Dashboard schoolId={schoolId} schoolName={schoolName} studentCount={studentCount} stats={dashboardStats} onAction={onAction} />; if (activeNav === "Élèves") return <StudentsPage schoolId={schoolId} onAction={onAction} />; if (activeNav === "Inscriptions") return <Registrations schoolId={schoolId} schoolSlug={schoolSlug} />; if (activeNav === "Classes") return <ClassesPage schoolId={schoolId} onAction={onAction} />; if (activeNav === "Pédagogie") return <Pedagogy schoolId={schoolId} />; if (activeNav === "Vie scolaire") return <Attendance schoolId={schoolId} />; if (activeNav === "Finances") return <Payments schoolId={schoolId} />; if (activeNav === "Documents") return <Documents schoolId={schoolId} />; if (activeNav === "Communication") return <Communications schoolId={schoolId} />; if (activeNav === "Équipe & rôles") return <Roles schoolId={schoolId} />; const module = moduleMap[activeNav] ?? moduleMap.Pédagogie; return <GenericModule schoolId={schoolId} title={activeNav} eyebrow={module.eyebrow} icon={module.icon} onAction={onAction} />; };
